...
# PODNAME: cyclic-scoring.pl
# ABSTRACT: Cyclopeptide Scoring
...
# Default options
my $input_file = 'cyclic-scoring-sample-input.txt';
my ( $debug, $help, $man );
...
printf "%s\n", cyclic_scoring( $peptide, @spectrum );
...
This script solves the Cyclopeptide Scoring Problem.

Input: An amino acid string I<Peptide> and a collection of integers I<Spectrum>.

Output: The score of I<Peptide> against I<Spectrum>, I<Score>(I<Peptide>,
I<Spectrum>).

=head1 EXAMPLES

    perl cyclic-scoring.pl

    perl cyclic-scoring.pl --input_file cyclic-scoring-extra-input.txt

    diff <(perl cyclic-scoring.pl) cyclic-scoring-sample-output.txt

    diff <(perl cyclic-scoring.pl --input_file cyclic-scoring-extra-input.txt) \
        cyclic-scoring-extra-output.txt

    perl cyclic-scoring.pl --input_file dataset_102_3.txt \
        > dataset_102_3_output.txt
